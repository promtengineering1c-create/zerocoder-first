...
class TimeEntry(tk.Entry):
    ...
    def on_write(self, *args):
        ...
        # 2. Если мы перешли границу часов, двигаем курсор вправо
        if len(s) == 4 and cursor_pos == 3:
            cursor_pos += 1

        self._last = s
        self.var.set(s)
        
        # 3. Возвращаем курсор на законное место
...
def get_current_tasks(only_current=True):
    ...
    with data_lock:
        if only_current:
            current_tasks = [task for task in tasks.values() if not task.done]  
        else:
            current_tasks = [task for task in tasks.values()]

    if current_tasks:
        current_tasks.sort(key=lambda x: x.deadline)
        for task in current_tasks:
            status_icon = "✅ " if task.done else ""
            task_listbox.insert(tk.END, f"{task.deadline}: {status_icon}{task.description}")      
...
def create_task_window(task_time, text):
    ...
    text_task.pack(side=tk.TOP, padx=10)
    ...
    button_done.pack(pady=10, padx=10)
